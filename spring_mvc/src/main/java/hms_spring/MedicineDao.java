...
public class MedicineDao {
	...
	public void update(MedicineCommand medicine, int stock) {
		jdbcTemplate.update("update MEDICINE set NAME=?, STOCK=? where NUM=?", medicine.getName(),
				medicine.getStock() + stock, medicine.getNum());
	}
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				PreparedStatement pstmt = con
						.prepareStatement("insert into MEDICINE (NUM, NAME) values(MEDICINE_SEQ.nextval, ?)");
				// 인덱스 파라미터 값 설정
